...
public class PatternKillauraMove extends AbstractCheck<PlayerMoveEvent> {
	...
	public void Check(PlayerMoveEvent event) {
		...
		if ((System.currentTimeMillis()
				- PatternKillauraAttack.lastAttack.getOrDefault(uuid, System.currentTimeMillis()) > 350L)
				|| PatternKillauraAttack.lastHit.get(uuid) == null) {
			return;
		}
		NessPlayer np = manager.getPlayer(p);
		List<Float> patterns = np.getPatterns();
		float offset = Utility.yawTo180F(
				(float) Utility.getOffsetFromEntity(event.getPlayer(), PatternKillauraAttack.lastHit.get(uuid))[0]);
		...
		if (patterns.size() >= 23) {
			...
			if (Math.abs(range - PatternKillauraAttack.lastRange.getOrDefault(uuid, 0.0f)) < 2) {
				...
				np.setViolation(new Violation("Killaura",
						"Result: " + Math.abs(range - PatternKillauraAttack.lastRange.getOrDefault(uuid, 0.0f))));
			}
